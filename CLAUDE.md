# wavesurfer.js (Wavvy's local fork)

Local fork of wavesurfer.js used by the Wavvy DAW. Globally npm-linked (not listed in wavvy's `package.json`).

## Link workflow

1. `cd wavesurfer.js && npm link` (once)
2. `cd ../wavvy && npm link wavesurfer.js` (once)

## Rebuild workflow

**Whenever any source files in this directory change:**

1. `npm run build` (in this dir)
2. `rm -rf ../wavvy/node_modules/.vite` (clear Vite cache)
3. Restart the Wavvy dev server

Skipping any of these causes stale code to be served to the browser.

## Build constraints

**Rollup CJS/UMD builds use `exports: 'default'`** — do not add named exports to `wavesurfer.ts` or the build will fail. Export new utilities from their own module files instead.

## Shadow DOM

WaveSurfer renders inside a shadow root (`src/renderer.ts`). External CSS **cannot** reach clip elements — styles must go in the shadow DOM `<style>` block inside `renderer.ts`.

- Clip elements have `class="ws-clip"`.
- **Clip frame uses `outline` with `outline-offset: -2px` (not `border`)** — zero layout impact, so the clip's inner canvas isn't offset from the wrapper top and the waveform centerline aligns with WaveSurfer's own canvases. Do not reintroduce a border on `.ws-clip`.

## Project duration lift

`WebAudioPlayer.setProjectDuration(seconds)` lifts the player's logical `duration` to `max(real audio duration, projectDuration)`. Wavesurfer propagates it automatically from `setOptions({ projectDuration })` and at the end of every `loadAudio` (`propagateProjectDuration()`) — hosts never call it directly.

**Propagation-order invariant (2026-08-12)**: in `setOptions`, `propagateProjectDuration()` must run **BEFORE** `renderer.setOptions(...)`. The renderer re-render synchronously emits `render` → wavesurfer re-emits `redraw`, and redraw listeners (TimelinePlugin gridline layout) read `getEffectiveDuration()` = `max(getDuration(), options.projectDuration)`. With a stale player lift, a projectDuration **shrink** (e.g. Wavvy's post-recording shrink-back) makes that read return the old larger duration while the wrapper is already sized for the new one — gridlines lay out at a compressed px-per-sec and stay misaligned until the next redraw (zoom). Regression test: `src/__tests__/project-duration-propagation.test.ts`. This is what lets a track whose buffer (e.g. Wavvy's silent placeholder) is shorter than the project keep advancing `currentTime`, not self-pause at the buffer end, and accept play/seek positions up to the timeline end. Rationale + host-side rules: wavvy's `docs/architecture/master-transport.md` ("Project duration lift").

**Renderer data-extent invariant**: waveform data is drawn at `audioDuration × pxPerSec`, never stretched across the (wider) projectDuration-based wrapper. `render` / `renderMultiCanvas` / `renderUpdate` all slice channel data against `dataWidth`/`dataTotal`, boundary tiles are drawn narrower, and tiles beyond the audio collapse to width 0. Do not reintroduce `offset / totalWidth` slicing — it draws the live-recording waveform ahead of the cursor whenever the project outgrows the recording buffer.

**Lift-feedback trap**: code that wants the *audio's* duration must use `getDecodedData()?.duration`, not `getDuration()` (lifted). `record.ts` `processPcmData` clip-mode cleanup does this; so does wavvy's `updateProjectDuration`.

## Decoder normalization is IN-PLACE

`Decoder.createBuffer` normalizes >1 peaks by MUTATING the caller's arrays. Never feed it a live, long-lived buffer without `skipNormalization = true` (third arg) or a copy — `updatePeaks` passes the record plugin's live `dataWindow` every 10ms tick, and repeated in-place normalization eroded drawn peaks by a different factor per tick phase (same-input recording tracks rendered different waveforms from identical data; fixed 2026-08-30). Guard: `src/__tests__/decoder-normalize.test.ts`.

## RecordPlugin capture anchor

`setCaptureAnchor(ctxTime)` anchors a take to the AudioContext clock: merged PCM (`src/record-align.ts` `mergeChunksToAnchor`) is trimmed/padded so sample 0 ≡ the anchor, and the live-waveform edge + duration clock derive from `ctx.currentTime − anchor` instead of wall-clock timers. `pushPcmChunk(chunk, startCtxTime?)` carries the chunk's audio-clock timestamp. `startRecording()` clears the anchor — hosts set it per take, AFTER startRecording, once the transport's `playAt` time is known. In anchored mode the live waveform is drawn from the timestamped chunks (`accumulateChunkPeaks`), NOT the analyser — same-input recorders render identical waveforms. The live preview applies the same `compensationTrimFrames` offset as `processPcmData`, so preview ≡ committed placement and nothing moves on stop (WVY-590). The `expectCaptureAnchor` option holds elapsed/waveform at 0 until the anchor lands (no wall-clock snap-back in the pre-anchor window). This is what keeps simultaneously recorded wavvy tracks sample-aligned; see wavvy's `docs/architecture/master-transport.md` "Recording capture anchor".

## TimelinePlugin

**Beat-space-only law for the beats-mode grid (2026-09-03)**: gridline classification (bar/beat/sub opacity) and pixel placement must derive from **beat indices**, never seconds. Classification: `subdivisionToBeatsFraction(sub)` `{num, den}` rationals (kept in lockstep with `subdivisionToSeconds`, triplets included) — `atBeat ⟺ (i·num) % den === 0`, `atBar ⟺ (i·num) % (4·den) === 0`. Placement: `beatsPos × pxPerBeat` with NO time rounding. The old seconds math (`|time % barDuration| < 0.001` and `Math.round(time*100)/100 × pxPerSec`) made line weights flicker and positions jitter ±1px during a tempo drag at any non-binary-exact tempo — the host's beat-anchored zoom holds px-per-beat constant, and only beat-space math preserves that on screen.

Has a `gridSubdivision` option and `setGridSubdivision()` method for variable grid density — used by Wavvy's per-track gridlines.

**Canvas grid (2026-09-04)**: grid lines render to ONE viewport-sized canvas per plugin instance (backing store 1 device-px tall, CSS-stretched vertically — vertical lines are uniform, so repaints never read the track height), repainted from the `scroll` event's own pixel args (+200px margin, skip while the window still covers the viewport) and `translateX`-positioned. The old per-line DOM divs + per-line `virtualAppend` scroll listeners (~29k divs per zoom step at narrow grids) are gone for grids; `virtualAppend` survives only for the notch/label bar, which is skipped ENTIRELY when `height: 0` (Wavvy's per-track config). A power-of-2 index step thins tiers whose spacing falls under 3 CSS px (extreme zoom-out) — classification math untouched. The layout snapshot (`gridLayout`) is captured once per `redraw` at `wrapper.scrollWidth / getEffectiveDuration()` — it does NOT track later width/duration changes, so anything that changes either value must end in a redraw that fires with both already consistent (see "Propagation-order invariant" above).

## ClipsPlugin drag ghost

Body drags are Ableton-style (since 2026-07-25): the clip element stays put and a translucent clone (`.ws-clip-ghost`, canvas bitmaps blitted once at drag start) tracks the pointer — snapped to the grid when the host's `setSnapConfig` is enabled — committing on release. Continuous `clip-drag` plugin event fires per pointer move; `clip-drag-end` still sees the final `startTime` on the block. Public per-clip API `showDragGhost(t, peer?)` / `hideDragGhost()` + readable `dragTargetTime` let the host preview group moves on non-dragged clips; the optional `peer: { color, label }` tints the ghost (colored outline + `.ws-clip-ghost-badge` initial) for a REMOTE collaborator's drag — local drags never pass it; the optional third `duration` arg overrides the ghost width (remote resize preview). Resizes fire a continuous `clip-resize` plugin event (block already at the in-progress extent) before `clip-resize-end`. Details: wavvy's `docs/architecture/clip-rendering.md` "Body-drag ghost".

## ClipsPlugin origin mark (wavvy WVY-652)

The clip label is a structured `div.ws-clip-label` row — optional hollow `span.ws-clip-origin` disc (collaborator initials, BEFORE the name so ellipsis never eats it), `span.ws-clip-name`, and a `span.ws-clip-origin-text` " · initials" tail that the renderer's `@container (min-width: 160px)` rule reveals (`.ws-clip` is `container-type: inline-size`). `addClip({ origin })` / `setOrigin(mark | null)` drive it; `setName` writes the name span ONLY (writing the label's `textContent` would delete the disc), and `getLabelElement()` returns the outer row. Hollow disc = origin (past tense); the solid `.ws-clip-ghost-badge` stays the live-drag tense. Pin: `src/__tests__/clips-origin-mark.test.ts`; wavvy side: `docs/architecture/clip-rendering.md` "Origin mark in the clip label".

## ClipsPlugin gain scale (wavvy WVY-660)

`addClip({ gainScale })` / `setGainScale(n)` set a per-clip **draw-time** amplitude multiplier (default 1; the host passes `10^(clipGainDb/20)`). All three waveform draw primitives — `drawChannelPeaks`, `drawChannelSampleLine`, `drawChannelTiledSampleLine` — scale by it and clip the drawn envelope at ±1 via `applyGainClip` (`src/clip-render-math.ts`), so the boosted waveform flattens against the box (Ableton clip view) instead of overflowing. Scaling the three primitives covers all five render paths (plain peaks, plain sample-line, both tilers, precomputed-peaks fallback); the peak/PCM computation helpers are deliberately untouched — **`peaks` and `pcm` are live host references and are never written** (they feed wavvy's merge fingerprints and saved state). `gainScale` is in the `lastPaintState` key *and* its persist block (otherwise the short-circuit swallows the repaint); `setGainScale` is the only writer and no-ops on a repeat value. Non-finite / zero / negative sanitizes to 1. Pin: `src/__tests__/clips-gain-scale.test.ts`; wavvy side: `docs/architecture/clip-rendering.md` "Gain-scaled waveform (WVY-660)".

## ClipsPlugin windowed content (`contentWindowed`, WVY-87)

`addClip({ renderContent, contentWindowed: true })` routes a custom renderer through the same viewport-sized, timeline-device-pixel-grid-quantized canvas window as the PCM path and calls it with a 5th arg, `ClipContentWindow { startSec, endSec, pxPerSecDevice, bitmapW }` (`src/clip-render-math.ts`, `computeContentWindow`). 5-arg contract: map `x = (t − startSec) × pxPerSecDevice`, ignore `contentW`, and treat a NEGATIVE `startSec` as the left-edge-drag lead-in — real content (earlier loop tiles / earlier trim source) must be painted there, because `repaintForResizeDrag` slides the canvas into it (slide origin `paintCanvasLeftCss`). Windowed clips get a paint short-circuit keyed on window geometry + loop/drag inputs; `setData`/`setRenderContent` invalidate it. Flag off (default) = the legacy full-clip 4-arg path, unchanged. Wavvy consumer + details: `src/utils/midi-note-preview.ts`, `docs/architecture/clip-rendering.md` "Render path dispatch" §3.

## Notes

- See `AGENTS.md` in this dir for guidance targeting other AI agents (Codex, Gemini, etc.).
- This file is the Claude Code equivalent.
